...
import (
	...
	"github.com/verma-kartik/Agora/queue"
...
type messageQueue struct {
	Name                   string
	messageInput           chan *message.Msg
	messageOutput          chan *message.Msg
	queue                  *queue.Queue
	metrics                chan<- *Metric
	closing                chan<- *string
...
func newMessageQueue(queueName string, metricsChannel chan<- *Metric, closingChannel chan<- *string) *messageQueue {
	...
	q.messageInput = make(chan *message.Msg)
...
func (q *messageQueue) Publish(givenMessage *message.Msg) {
